...
void probe_offset_wizard_menu() {
  ...
  if (LCD_HEIGHT >= 4)
    STATIC_ITEM_F(F(STRINGIFY(Calibrate With Paper)), SS_CENTER|SS_INVERT);
  ...
  SUBMENU_f(F(STRINGIFY(Move Z 0.5)), MSG_MOVE_N_MM, []{ _goto_manual_move_z(float(0.05)); });
